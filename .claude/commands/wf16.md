# /wf16 - Fast-Iteration TDD on Modern Primitives (Tier-Auto, Split-TDD, No-Worktree Default)

**Version 16.3**: Antigravity edition. `-g` now invokes `agy -p` (Antigravity CLI subprocess), the successor to Gemini MCP (Gemini CLI sunsets 2026-06-18). `-c` (Codex MCP) unchanged. No-flag defaults: tier-auto, split TDD, no worktree, no reviewers, no human gate, no auto-commit.

**Tag:** `[tier-auto | split-tdd-default | no-worktree-default | optional MCP | rewind-discard | no-auto-commit]`
...
**Combinable.** Examples:
- `/wf16 fix typo in README` → auto picks `micro` → 1 agent, no worktree, no ceremony
- `/wf16 implement email validator` → auto picks `small` → 1 split-TDD pair + coverage cop
- `/wf16 implement OAuth refresh flow` → auto picks `full` → split TDD + 3 cops, still no worktree unless `--worktree`
- `/wf16 --tier=full --worktree -cg --commit implement payment flow` → kitchen sink
...
if [ "$HOOKS_INSTALLED" = "0" ]; then
  ...
  echo "Run /wf16-install-hooks (sibling command) to add them. Hooks are advisory; workflow proceeds either way."
fi
```
...
```bash
ITER_COUNT=$(grep -c '^GREEN_ITER\|^ITER' /tmp/wf16-tdd-output.txt)
FINAL_EXIT=$(grep 'EXIT_CODE:' /tmp/wf16-tdd-output.txt | tail -1 | awk '{print $2}')
...
```bash
$TEST_CMD 2>&1 | tee "$WT_PATH/.wf16-final-verification.txt"
echo "EXIT_CODE: $?"
```
...
Output: `ORCHESTRATION COMPLETE (wf16 v16.3, tier=$TIER)`
...
| One-shot exploration, can't define quick test | use `/wf12` or `/wf14` instead | wf16 requires a quick test |

## When to Use wf16 vs wf15

| Situation | Use |
|---|---|
| Modern Claude Code (Agent + worktree + AskUserQuestion + ToolSearch) | wf16 |
| Cost-sensitive (Sonnet cops + tiered scaling) | wf16 |
| Merge gated behind explicit `--commit` | wf16 |
| Older Claude Code without native Agent | wf15 |
| Need wf15's `claude -p` subprocess semantics | wf15 |
