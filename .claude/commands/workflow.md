...
**WF_VERSION:** `v0.28` · **WF_COMMITTED:** `19-jun-2026` · **Tag:** `[tier-auto | split-tdd | codex-default | rewind-discard | no-auto-commit | evidence-graded-gates | timing-receipt | segment-timing | assisted-by-trailer | longitudinal-ratchet | reviewer-timeout]`

**First line of every run must be, verbatim:** `workflow v0.28 (19-jun-2026)` — derived from the two values above. Bump both when the workflow body changes meaningfully.
...
The `-g` reviewer runs through the repository-owned **agy bridge**: use `mcp__agy__agy_ask`, or `~/.local/bin/agy-ask` when the MCP tool is not exposed. The bridge calls `agy` first and uses the configured local Gemini-compatible fallback only when the primary route fails. Never substitute raw `agy`, `gemini`, a browser, Vertex, or Gemini API.

The bridge owns model selection and fallback internally. Its endpoint and model are configured with `AGY_FALLBACK_BASE_URL` and `AGY_FALLBACK_MODEL`; credentials remain inside the fallback service. If Antigravity is unavailable it auto-routes and prefixes the response with the actual route; that is a valid Gemini response, not a downgrade. If bridge behavior looks stale, restart Claude Code so the MCP server reloads.

On MISSING: continue with the missing reviewer disabled for execution (its internal flag forced false) and print the downgrade loudly (default — `ALLOW_MCP_DOWNGRADE=true` since v16.2; with Codex default-on this means `CODEX REVIEW SKIPPED - MCP missing` must appear in the final output). **Record it at the gate as `UNREACH (MCP missing)`, not `N/A`** — "forced false" here means "tried, unreachable," which is the same downgrade family as a timeout; `N/A` is reserved for a reviewer the user intentionally did not run (`--no-codex`, or not active for the tier). `--abort-on-missing-mcp` restores the abort behavior. For `-g`, "missing" means neither `mcp__agy__agy_ask` nor executable `~/.local/bin/agy-ask` is available. **A reviewer that does not reply within the 5-minute cap is treated as unreachable too** — same downgrade path; see "Reviewer wall-clock cap" immediately below.
...
| `-g` | Bridge call | `mcp__agy__agy_ask`, or `~/.local/bin/agy-ask` when MCP is not exposed. The bridge auto-routes to its configured fallback and returns Gemini text for verdict parsing. |
...
if [ -z "$LEDGER" ] || [ ! -f "$LEDGER" ]; then
  echo "⏱ workflow v0.28 tier=$TIER | TOTAL UNVERIFIED (start stamp not found)"
else
  ...
  echo "⏱ workflow v0.28 tier=$TIER | $SH → $EH | TOTAL $H"
...
```text
⏱ workflow v0.28 tier=small | 2026-06-11 14:02:11 → 2026-06-11 14:19:48 | TOTAL 17m 37s
...
Output (final line): `ORCHESTRATION COMPLETE (workflow v0.28, tier=$TIER)`
