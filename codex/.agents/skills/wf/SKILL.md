...
7. Review gate:
   ...
   - Call `mcp__agy__agy_ask` with the review prompt and a bounded timeout when supported. If the MCP tool is not exposed, invoke `~/.local/bin/agy-ask` with the same prompt. Both enter the same repository-owned router, which first tries Antigravity `agy`, then auto-routes to its configured local Gemini-compatible fallback.
   - Treat a response prefixed `[agy quota exhausted — auto-routed …]` as a valid Gemini reviewer response, not as a degradation. Record the route in the review summary.
   - If the agy response is truncated, continue with `mcp__agy__agy_continue` or `~/.local/bin/agy-ask continue` rather than starting a new reviewer pass.
   - Do not invoke raw `agy`, `gemini`, a browser, Vertex, or Gemini API, and do not replace a requested `-g` review with Claude, Codex, or inline self-review. Only degrade when both bridge transports are unavailable or the bridge errors after trying its own fallback.
